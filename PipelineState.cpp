...
void PipelineState::SetVS(std::wstring filePath)
{
	...
	auto hr = D3DCompileFromFile(
		...
		0,
		m_pVsBlob.GetAddressOf(), // 対象ポインター
		errorBlob.GetAddressOf() // エラーメッセージ用ポインター
	);
...
void PipelineState::SetPS(std::wstring filePath)
{
	// ピクセルシェーダー読み込み
	auto hr = D3DCompileFromFile(
		filePath.c_str(), // シェーダー名 ピクセルシェーダー
		nullptr, // defineなし
		D3D_COMPILE_STANDARD_FILE_INCLUDE, // インクルードはデフォルト
		"PSmain", // PS関数
		"ps_5_0", // 対象シェーダー
		D3DCOMPILE_DEBUG | D3DCOMPILE_SKIP_OPTIMIZATION, // デバッグ用、最適化なし
		0,
		m_pPSBlob.GetAddressOf(), // 対象ポインター
		errorBlob.GetAddressOf() // エラーメッセージ用ポインター
	);
